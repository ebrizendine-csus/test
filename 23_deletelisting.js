...
describe('Delete Listing Test', function () {
  this.timeout(30_000);
  ...
  it('deletes the first listing in the table', async () => {
    ...
    const firstRow = await driver.wait(async () => {
      ...
      return false;
    }, 10_000, 'No visible listing rows appeared');

    const listingId = await firstRow.findElement(By.css('td:first-child')).getText();


    const deleteBtn = await firstRow.findElement(By.css('button.delete-listing-btn'));
    ...
    const confirmBtn = await driver.wait(until.elementLocated(By.id('confirmDeleteBtn')), 5_000);
    await driver.wait(async () => {
      if (!(await confirmBtn.isDisplayed())) return false;
      const { width, height } = await confirmBtn.getRect();
      return width > 0 && height > 0;
    }, 5_000, 'confirmDeleteBtn never became interactable');
    ...
    const leftovers = await driver.findElements(
      By.xpath(`//tbody/tr/td[1][text()="${listingId}"]`)
    );
    assert.strictEqual(
      leftovers.length,
      0,
      `Listing ID ${listingId} is still present after deletion`
    );
  });
});
